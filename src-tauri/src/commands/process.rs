...
pub async fn check_port_in_use(port: u16) -> Result<bool, String> {
    ...
    // 使用 openclaw health 检查当前配置的 gateway 端口是否在运行。
    if port == shell::get_gateway_port() {
    ...
    match shell::run_command_output("node", &["--version"]) {
        ...
        Err(e) => {
            if crate::utils::platform::is_windows() {
                if let Ok(version) = shell::run_wsl_bash_output("node --version 2>/dev/null") {
                    let version = version.trim().to_string();
                    if !version.is_empty() {
                        info!("[进程检查] WSL Node.js 版本: {}", version);
